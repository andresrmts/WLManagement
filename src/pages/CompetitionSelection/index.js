...
const CompetitionSelection = ({ adminToggle, onSearchChange }) => {
  ...
  const availableComps = competitions.filter(
    (competition) => !yourComps.some((comp) => competition.name === comp.name)
  );
  ...
  return (
    <div>
      ...
      >
        You are signed in as {userName}. Please Create a new Competition or join
        an existing competition.
      </p>
      <div>
        <div
          className="fl w-100 w-50-ns pa2 tc"
          style={{ display: "flex", justifyContent: "center" }}
        >
          <Link
            to={routes.competitioncreation.path}
            className="pointer f6 link dim br-pill ba ph3 pv2 mb2 mid-gray"
          >
            New Competition
          </Link>
        </div>
        <div
          className="fl w-100 w-50-ns pa1 black-80"
          style={{ display: "flex", justifyContent: "center" }}
        >
