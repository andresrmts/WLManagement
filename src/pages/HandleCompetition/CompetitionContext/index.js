...
const CompetitionProvider = ({ children, userName }) => {
  ...
  const editAthleteWeight = (athleteName) => {
    ...
    setRegisteredAthletes((pS) =>
      pS.map((ath) => (athleteName === ath.name ? { ...ath, weight } : ath))
    );
  };
  ...
  const nextLift = () => {
    setLift("cnj");
    setRegisteredAthletes((pS) =>
      pS.map((ath) => Object.assign(ath, { attempt: 0 }))
    );
  ...
  const goToNextAttempt = (athlete, weight, attempt) => {
    ...
    if (
      verdict.result > 0 &&
      verdict.votes === 3 &&
      registeredAthletes[index].attempt < 3
    ) {
      registeredAthletes[index].result[lift].push(weight);
      setRegisteredAthletes((pS) =>
        pS.map((ath) =>
          ath.name === athlete
            ? Object.assign(ath, { [lift]: weight++, attempt: attempt++ })
            : ath
        )
      );
      ...
      setRegisteredAthletes((pS) =>
        pS.map((ath) =>
          ath.name === athlete
            ? Object.assign(ath, { attempt: attempt++ })
            : ath
        )
      );
      setVerdict({ result: 0, votes: 0 });
  ...
  const approveRemove = (decision, name, role) => {
    ...
    if (decision === "yes" && role === "judge") {
      if (
        acceptedRegistrations.filter((judge) => judge.role === "judge").length <
        3
      ) {
...
const useCompetitionContext = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "This function can only be used within CompetitionProvider"
    );
  }
  return context;
};
